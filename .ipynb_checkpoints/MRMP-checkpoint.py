...
from data.ucmerced import UCMERCED
from data.aid import AID
...
# Arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser("PyTorch Aerial Training")
parser.add_argument("--dataset", type=str, default="ucmerced",
                    help="dataset (ucmerced, aid)")
...
parser.add_argument("--num_iter_per_epoch", type=int, default=400, help="iters/epoch")
...
parser.add_argument("--T", type=float, default=0.7, help="sharpening temperature")
parser.add_argument("--ES_step", type=int, default=5, help="early stop patience")
...
if os.path.exists(txtfile) and args.load is None:
...
def exp_func_fixed_c(x, a, b):
    """Log-space model: log(KL - c) = log(a) - b*x  ->  KL = a*exp(-b*x) + c."""
    return np.log(a) - b * x

def pred_round(a, b, c):                                    
    """Round where the fitted KL curve equals 2*c (one floor above the minimum)."""
...
def train(train_loader, epoch, model, optimizer):
    ...
    for i, (images, labels, s_labels, indexes) in tqdm(
        enumerate(train_loader), total=min(len(train_loader), args.num_iter_per_epoch),
...
def purify_dataset(model, ema, nd, dataset, s_dataset, json_f):
    ...
    with torch.no_grad():
        for images, labels, s_labels, idxs in tqdm(loader, desc="Purifying"):
            ...
            pur = nd(c_labels, labels)

            # KL divergence between purified and current noisy labels
            mean_kl += F.kl_div(
                pur.clamp(min=1e-8).log(), labels.clamp(min=1e-8), reduction="batchmean"
            ).item()
            kl_batches += 1
...
for r in range(args.start_round, args.n_round + 1):
    ...
    # convnext_base for aerial; convnext_small for CIFAR
    if args.dataset in ("ucmerced", "aid"):
    ...
    # argmax(-1).reshape(-1) works for both (N,C) CIFAR and (N,1,C) aerial label shapes
    matching = (
        train_dataset.train_noisy_labels.argmax(-1).reshape(-1)
        == train_dataset.train_labels.argmax(-1).reshape(-1)
    )
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        with open(txtfile, "a") as f:
            f.write(f"{epoch} {train_acc:.4f} {train_loss:.4f} "
    ...
    # Purify dataset
    # ------------------------------------------------------------------
    json_f = purify_dataset(
        cnn, cnn_ema, nd, base_dataset, train_dataset, json_f)
