...
from data.miniclothing1m import MiniClothing1M
...
parser.add_argument("--dataset", type=str, default="miniclothing1M", help="dataset")
...
model_str = (
    f"{args.dataset}_MRML_beta{args.beta}_tau{args.T}"
)

# ---------------------------------------------------------------------------
# Dataset
# ---------------------------------------------------------------------------
if args.dataset=='miniclothing1M':
    input_channel=3
    num_classes=14
    ...
    try:
        ...
        root = '../mini_clothing1M'
        train_dataset = MiniClothing1M(root=root,
                                split="total", 
...
for r in range(args.start_round, args.n_round + 1):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        print(
            ...
            f"Val(meta): {meta_acc:.2f}% (EMA {meta_acc_ema:.2f}%) | "
            f"Loss: {meta_loss:.4f} (EMA {meta_loss_ema:.4f})"
        )
