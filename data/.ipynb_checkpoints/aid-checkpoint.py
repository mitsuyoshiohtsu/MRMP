...
class AID(data.Dataset):
    base_folder = 'AID'
    url = "https://www.kaggle.com/api/v1/datasets/download/jiayuanchengala/aid-scene-classification-datasets"
    filename = "AID-dataset.zip"
    ...
    def __init__(self, root, train=True,
        ...
        classname = os.listdir('./data/AID')
        img_paths = []
        classes_img = []
        for cls in classname:
            for sub in os.listdir(f'./data/AID/{cls}'):
                img_paths.append(f'./data/AID/{cls}/{sub}')
    ...
    def download(self):
        curl_command = [
            "curl", "-L", "-o", "./data/AID-dataset.zip",
            "https://www.kaggle.com/api/v1/datasets/download/jiayuanchengala/aid-scene-classification-datasets",
        ]
        subprocess.run(curl_command, capture_output=True, text=True)

        zip_path = './data/AID-dataset.zip'
        extract_path = './data/'
