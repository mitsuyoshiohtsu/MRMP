        ...
        if self.train:
            ...
            if noise_type in ["pairflip", "symmetric"]:
                ...
                with torch.serialization.safe_globals([np.core.multiarray._reconstruct]):
                    cifer10N = torch.load('../CIFAR-10_human.pt', weights_only=False)
                self.train_noisy_labels = cifer10N[noise_type]
                self.actual_noise_rate = (self.train_noisy_labels!=self.train_labels).mean()
            ...
            self.train_noisy_labels = np.eye(num_classes)[self.train_noisy_labels]
            ...
            num_classes = np.max(self.test_labels) + 1
            self.test_labels = np.eye(num_classes)[self.test_labels]
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            if self.noise_type is not None:
                img, target, src = self.train_data[index], self.train_noisy_labels[index], self.train_noisy_labels_s[index]
            else:
    ...
    def __init__(self, root, train=True,
        ...
        if self.train:
            ...
            if noise_type != "noisy_label":
                ...
                self.train_labels=np.asarray([[self.train_labels[i]] for i in range(len(self.train_labels))])
                with torch.serialization.safe_globals([np.core.multiarray._reconstruct]):
                    cifer100N = torch.load('../CIFAR-100_human.pt', weights_only=False)
                self.train_noisy_labels = cifer100N[noise_type]
                self.actual_noise_rate = (self.train_noisy_labels!=self.train_labels).mean()
                self.train_noisy_labels=[i for i in self.train_noisy_labels]
                _train_labels=[i[0] for i in self.train_labels]
                self.noise_or_not = np.transpose(self.train_noisy_labels)==np.transpose(_train_labels)
